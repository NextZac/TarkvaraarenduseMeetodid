...
export class Migration20241009103444 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table \`form_answer\` (\`id\` integer not null primary key autoincrement, \`answer\` text not null, \`form_id\` integer not null, \`updated_at\` datetime not null, constraint \`form_answer_form_id_foreign\` foreign key(\`form_id\`) references \`form\`(\`id\`) on update cascade);`);
    this.addSql(`create unique index \`form_answer_form_id_unique\` on \`form_answer\` (\`form_id\`);`);
